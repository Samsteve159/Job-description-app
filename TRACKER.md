...
| Date | What happened |
|---|---|
| 4 Sep 2026 | The writer became a loop. It tailors, reads which of the posting's requirements no bullet answers and which bullets the gates refused, and repairs only those, up to three rounds. Repairs are cheap because everything that passed is left alone: 284s for the first pass, then 24s and 13s. It also writes for a field now, picked deterministically from the title and required terms, because a consulting reader and a data reader want different evidence about the same work. Live run closed one of two gaps, then stopped and reported the other as real. Also bounded what extract may return, after a long posting overran its budget. Tests 753 to 784 |
| 3 Sep 2026 | The model every primary stage ran on reached end of life at 08:00 UTC and started returning 410. Re-probed, re-pointed, and found the paid fallback was failing too: it was spending its whole token budget on extended thinking and returning no text. Both fixed. Then his own working rules were built into the app as enforced checks rather than prompt lines: qualifications he does not hold, domains with no entry on his record, justified body text, the withheld-clients line, and a footer on two-pagers. Tests 687 to 753 |
